...
// Função SEGURA para inserir pedido (SEMPRE via API route do servidor)
export const insertPedido = async (pedido: Omit<PedidoSupabase, 'id' | 'created_at' | 'updated_at'>) => {
  console.log('🚀 SALVANDO PEDIDO NO SUPABASE - MODO SERVIDOR')
  console.log('📦 Salvando 1 itens no banco...')
  console.log('🚀 Tentando inserir pedido:', pedido)
  
  try {
    // SEMPRE usar a API route do servidor para evitar problemas de DNS no cliente
    ...
    console.log('✅ Pedido inserido com sucesso via servidor:', result.data)
    return result.data
  } catch (error: any) {
